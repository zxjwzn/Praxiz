...
class FriendListItem(ModelBase):
    ...
    account_id: str = Field(..., alias="accountId", description="好友账号 编号。")
    nickname: str = Field(..., description="好友昵称。")
    avatar: str = Field(..., description="好友头像 链接。")
    head_frame_id: int = Field(..., alias="headFrameId", description="头像框 编号。")
    ...
    game_status: str = Field(..., alias="gameStatus", description="游戏状态 结构化文本 字符串。")
...
class GetFriendListResponse(ResponseModel[FriendListResponseData]):
    """获取好友列表响应。"""