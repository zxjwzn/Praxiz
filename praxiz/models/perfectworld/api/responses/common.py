...
class SeasonStatsMapInfo(ModelBase):
    ...
    id: str = Field(..., description="地图 编号。")
    name_en: str = Field(..., description="地图英文名。")
    name_cn: str = Field(..., description="地图中文名。")
    logo: str = Field(..., description="地图 标识图 链接。")
    image: str = Field(..., description="地图图片 链接。")
    bp_color: str = Field(..., description="禁选 彩色图 链接。")
    bp_gray: str = Field(..., description="禁选 灰色图 链接。")
    bp_ladder: str = Field(..., description="天梯图标 链接。")
    bp_ladder_ban: str = Field(..., description="天梯禁用图标 链接。")
    bp_camp: str = Field(..., description="阵营图标 链接。")
    background: str = Field(..., description="背景图 链接。")
    download_type: str = Field(..., description="下载类型代码。")
    workshop_id: str = Field(..., description="创意工坊 编号。")
    url: str = Field(..., description="附加 链接，当前样本为空。")
...
class LadderTeamPlayer(ModelBase):
    ...
    player_id: str = Field(..., description="玩家 编号。")
    is_lord: bool = Field(..., description="是否为房主。")
    slot_id: int = Field(..., description="槽位 编号。")
    ...
    role_card_id: int = Field(..., description="角色卡 编号。")
...
class TeamSlotInfo(ModelBase):
    ...
    player_id: str = Field(..., description="槽位占用玩家 编号。")
...
class TeamAdvancedSetup(ModelBase):
    ...
    need_demo: bool = Field(..., description="是否需要 回放。")
...
class LadderTeamInfo(ModelBase):
    ...
    no_demo: bool = Field(..., description="是否不保存 回放。")
    bp_modes: list[int] = Field(..., description="禁选 模式列表。")
    me_match_mode: int = Field(..., description="匹配扩展 匹配模式。")
    me_begin_time: int = Field(..., description="匹配扩展 开始时间。")
    me_end_time: int = Field(..., description="匹配扩展 结束时间。")
...
class WeaponInfo(ModelBase):
    ...
    image: str = Field(..., description="武器图片 链接。")
    user_page_image: str = Field(..., description="用户页展示图 链接。")
...
class SeasonStatsMapItem(ModelBase):
    ...
    user_id: str = Field(..., description="用户 平台编号。")
    ...
    rws_avg: str = Field(..., description="回合贡献 平均值。")
    adpr: str = Field(..., description="场均伤害 场均伤害值。")
    ...
    match_mvp_num: str = Field(..., description="比赛 全场最佳 次数。")
    ...
    pri_avg: str = Field(..., description="表现指数 平均值。")
...
class SeasonStatsWeaponItem(ModelBase):
    ...
    total_rounds_kpr: int | None = Field(None, description="每回合击杀 总回合数样本，部分武器存在。")
    total_rounds: int | None = Field(None, description="总回合数，部分武器存在。")
    total_kills_kpr: int | None = Field(None, description="每回合击杀 统计击杀数，部分武器存在。")
...
class SeasonStatsLadder(ModelBase):
    ...
    id: str = Field(..., description="赛季统计记录 编号。")
    ...
    max_star: int | None = Field(..., description="赛季最高星数，样本中为 空值。")
    all_season_max_score: int | None = Field(..., description="历史赛季最高分，样本中为 空值。")
    all_season_max_star: int | None = Field(..., description="历史赛季最高星数，样本中为 空值。")
    ...
    rws_avg: float = Field(..., description="回合贡献 平均值。")
    adpr: float = Field(..., description="场均伤害 场均伤害值。")
    pri_avg: float = Field(..., description="表现指数 平均值。")
    ...
    match_mvp_num: int = Field(..., description="比赛 全场最佳 次数。")
    mvp_num: int = Field(..., description="全场最佳 次数。")
