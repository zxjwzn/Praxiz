...
class CurrentUserInfoData(ModelBase):
    ...
    id: str = Field(..., description="当前登录用户的 SteamID。")
    nickname: str = Field(..., description="当前用户昵称。")
    avatar: str = Field(..., description="头像 URL。")
    zqid: str = Field(..., description="完美平台站内 ID，具体含义待补充。")
    token: str = Field(..., description="当前登录令牌。")
    jt: str = Field(..., description="JWT 或会话凭证字符串。")
    ...
    ip_addr: str = Field(..., alias="ipAddr", description="当前登录 IP 地址。")
    ...
    has_charged: bool | None = Field(..., alias="hasCharged", description="是否已充值，样本中为 null。")
    has_used_discount: bool | None = Field(..., alias="hasUsedDiscount", description="是否已使用折扣，样本中为 null。")
    login_method: str = Field(..., description="登录方式标识。")
    is_vip: bool = Field(..., alias="isVip", description="是否为 VIP 用户。")
    vip_level: int = Field(..., alias="vipLevel", description="VIP 等级。")
...
class CurrentUserInfoResponse(GenericApiResponse[CurrentUserInfoData]):
    """当前登录用户信息响应。"""