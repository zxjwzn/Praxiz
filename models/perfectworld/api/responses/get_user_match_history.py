...
class MatchHistoryScore(ModelBase):
    ...
    team_id: str = Field(..., description="队伍 ID。")
...
class MatchHistoryMatchStart(ModelBase):
    ...
    ip: str = Field(..., description="服务器 IP 整数表示。")
...
class MatchHistoryItem(ModelBase):
    ...
    team_id: str = Field(..., description="队伍 ID。")
    svr_id: str = Field(..., description="服务器 ID。")
    ...
    replay_expired: bool = Field(..., description="Demo 是否已过期。")
    replay_available: bool = Field(..., description="Demo 是否可用。")
...
class MatchHistoryPayload(ModelBase):
    ...
    list: list[MatchHistoryItem] = Field(..., description="比赛历史列表。")
...
class GetUserMatchHistoryResponse(GenericApiResponse[MatchHistoryEnvelope]):
    """获取比赛历史响应。"""